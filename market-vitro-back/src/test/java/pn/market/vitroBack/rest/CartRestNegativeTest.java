...
class CartRestNegativeTest {
    ...
    void createCartOfUser() {
        Mockito.when(cartService.createOrUseCartOfUser(1L, 1L))
                .thenReturn(cartMono);
        Assertions.assertNotEquals(Mono.empty(), cartService.createOrUseCartOfUser(1L, 1L));
    }
    ...
    void getTotalSumOfCart() {
        Mockito.when(cartService.getTotalSumOfCart(1L))
                .thenReturn(longMono);
        Assertions.assertNotEquals(Mono.empty(), cartService.getTotalSumOfCart(1L));
    }
    ...
    void removeCartOfUser() {
        Mockito.when(cartService.removeFromCartOfUser(1L, 1L))
                .thenReturn(itemMono);
        Assertions.assertNotEquals(Mono.empty(), cartService.removeFromCartOfUser(1L, 1L));
    }
}